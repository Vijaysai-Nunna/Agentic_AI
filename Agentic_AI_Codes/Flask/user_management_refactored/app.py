...
def register_user():
    ...
    # Step 2.1 - Check if user already exist  in database 

    email = data["email"]
    user = UserHelper.find_user_by_email(email)

    if user:
        return jsonify({"Message": f"User Already exists with email {email} "}), 200
    ...
    UserHelper.save_users (users) # Save users collection back to Storage -> JSON
    return jsonify({"Message":"User Added Succefully"}), 200
...
def change_password():
    ...
    try:
        data = request.get_json()
        if not data:
            return jsonify({"message": "Please pass new and old password along with email."}), 400
        ...
        for u in users:
            ...
            app.logger.info(f"Comparing emails: input='{email}' stored='{user_email}'")
            
            if user_email == email:
                app.logger.info("Email match found")
                if u.password == old_password:
                    u.password = new_password
                    UserHelper.save_users(users)
